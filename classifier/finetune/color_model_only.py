...
from finetune.model import get_model, clip_predict
...
# =============================== dataset ================================
# all data
annotation = pd.read_csv('./data/color_annotation.csv')
...
model = get_model(model_name, class_num=args.class_num).to(device)
...
def train(model, train_iter, val_iter, epochs, optimizer, scheduler, loss):
    ...
    for epoch in range(epochs):
        ...
        for (images, img_paths, labels) in tqdm(train_iter):
            ...
            y_pred_softmax = torch.nn.functional.softmax(y_pred, dim=1).to(torch.float32)  # (bz,17)
            #  ## loss
        ...
        if acc > min_acc:
            min_acc = acc
            print("model saved!")
            torch.save(model.state_dict(), './models/color/'+model_name+'_model_only_CE.pth')
