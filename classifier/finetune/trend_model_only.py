...
annotation = annotation.loc[:, ['file_name', 'Trend']]
annotation = annotation.loc[annotation.Trend != 100, ['file_name', 'Trend']]
...
def train(model, train_iter, val_iter, epochs, optimizer, scheduler, loss):
    ...
    for epoch in range(epochs):
        ...
        if acc > min_acc:
            min_acc = acc
            print("model saved!")
            torch.save(model.state_dict(), './models/trend/'+model_name+'_model_only_focal.pth')
