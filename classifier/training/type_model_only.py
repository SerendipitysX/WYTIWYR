...
# =============================== dataset ================================
annotation = pd.read_csv('./data/annotation_18_30k.csv')
annotation = annotation.loc[:, ['file_name', 'Type']]
...
def train(model, train_iter, val_iter, epochs, optimizer, scheduler, loss):
    ...
    for epoch in range(epochs):
        ...
        if acc > min_acc:
            min_acc = acc
            print("model saved!")
            torch.save(model.state_dict(), './models/category/'+model_name+'_model_only_Focal_18.pth')
